...
url_b64_principal = 'aHR0cHM6Ly9iaXQubHkvM2FDV0hIeg=='
url_principal = base64.b64decode(url_b64_principal)
#######################################################################################
####  TITULO DO MENU  #################################################################
title_menu = '[B][COLOR white]BEM-VINDOS AO[/COLOR] [COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B]'
#######################################################################################
####  MENSAGEM DO STATUS BASE 64 ##############################################################
url_b64_mensagem = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2VNVGJaczNo'
...
url_b64_branco = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3LzQyTVZrOEx0'
url_branco = base64.b64decode(url_b64_branco)
#######################################################################################
###  DESCRIÇÃO DO ADDON ###############################################################
### OBS: PARA SALTAR UMA LINHA UTILIZE \n #############################################
url_b64_title_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2N0eEJic3FZ'
...
useragent = base64.b32decode('\x49\x5a\x53\x57\x34\x32\x4c\x59\x49\x5a\x57\x47\x53\x36\x42\x50\x47\x51\x58\x44\x4f\x4c\x52\x52')
##############################################################################


#### MENU VIP ################################################################
titulo_vip = '[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B] [COLOR firebrick][B](PREMIUM)[/B][/COLOR]'
thumbnail_vip = 'https://i.imgur.com/8bhsrFo.png'
fanart_vip = 'https://i.imgur.com/oE0ZDRN.png'
#### DESCRIÇÃO VIP ###########################################################
url_b64_vip_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0NXbllmcWp3'
url_vip_descricao = base64.b64decode(url_b64_vip_descricao)
#### DIALOGO VIP - SERVIDOR DESATIVADO - CLICK ###################################
url_b64_vip_dialogo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L242UVhaa2tH'
url_vip_dialogo = base64.b64decode(url_b64_vip_dialogo)


### DIALOGO
dialog1 = '[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B] - Selecione uma opçăo:'
...
url_b64_bem_vindo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0R4UjR6QTRO'
url_mensagem_bem_vindo = base64.b64decode(url_b64_bem_vindo)
## MENU CONFIGURAÇÕES
menu_configuracoes = '[B][COLOR white]CONFIGURAÇÕES[/COLOR][/B]'
url_b64_desc_configuracoes = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L042MjhKckhu'
icon_config = 'https://i.imgur.com/CUCWLoe.png'
url_desc_configuracoes = base64.b64decode(url_b64_desc_configuracoes)
## MENU CHECK UPDATE
menu_CheckUpdate = '[B][COLOR firebrick]CHECK [/COLOR][/B][B][COLOR white] UPDATE[/COLOR][/B]'
url_b64_desc_CheckUpdate = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L1BFeThYMU5r'
icon_upd = 'https://i.imgur.com/nK6Atwm.png'
url_desc_CheckUpdate = base64.b64decode(url_b64_desc_CheckUpdate)
...
def addon_log(string):
    if debug == 'true':
       # xbmc.log("[addon.live.SimpleKore Lists-%s]: %s" %(addon_version, string))
        xbmc.log("[FenixFlix-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
	       ...
	       #### HEADER PADRAO
               #headers = {'User-agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:19.0) Gecko/20100101 Firefox/19.0'}
               headers = {'User-agent' : 'HVCXUSLINZFXIZ2FPJVWG4SQGEZE2S2XNVVDMQLJLFDVM2S2O5HD2==='}
            req = urllib2.Request(url,None,headers)
            response = urllib2.urlopen(req)
            data = response.read()
            response.close()
...
def contador():
    ...
    "Referer": "plugin.video.FenixFlix-4.7.1",
    "Connection": "keep-alive" 
    }
    request = urllib2.Request("https://whos.amung.us/pingjs/?k=f6anhfa33p", headers=request_headers)
...
def CheckUpdate(msg): #54
	try:
		...
		if uversao != Versao:
			...
			xbmcgui.Dialog().ok('[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B]', "O addon já esta com a última versão: [COLOR firebrick]4.7.1[/COLOR] de "+Versao+"\nAs atualizações normalmente são automáticas, caso não atualize, baixe o zip atualizado em:\n[COLOR firebrick]www.fenixflix.ml[/COLOR]\nUse esse recurso caso não atualize!")
			xbmc.executebuiltin("XBMC.Container.Refresh()")
	except urllib2.URLError, e:
		if msg==True:
			xbmcgui.Dialog().ok('[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B]', "Você já esta com a última versão!!!")
...
def module_import(url):
    ...
    return response
# MODULOS
exec(module_import(base64.b32decode('NB2HI4DTHIXS64DBON2GKYTJNYXGG33NF5ZGC5ZPIVHG2ZCTGBMEW===')))
...
def vip():
    ...
    if vip_menu == 'true':
        if tipo_servidor=='PREMIUM':
            url = 'http://assistaiptvplay.com:8880/get.php?username=%s&password=%s&type=m3u_plus&output=%s'%(username_vip,password_vip,saida_transmissao)
            addDir(titulo_vip,url,1,thumbnail_vip,fanart_vip,descricao(url_vip_descricao),'','','','','','','','','','','','','','','','')
        if tipo_servidor=='Desativado':
            addDir(titulo_vip,'',42,thumbnail_vip,fanart_vip,descricao(url_vip_descricao),'','','','','','','','','','','','','','','','')

#SKindex():
#exec(module_import(base64.b32decode('NB2HI4DTHIXS64DBON2GKYTJNYXGG33NF5ZGC5ZPHE2HM532G5EEU===')))
def SKindex():
    addon_log("SKindex")
    CheckUpdate(False)
    ...
    addDir(menu_configuracoes,'',41,icon_config,FANART,descricao(url_desc_configuracoes),'','','','','','','','','','','','','','','')
    addDir(menu_CheckUpdate,'',44,icon_upd,FANART,descricao(url_desc_CheckUpdate),'','','','','','','','','','','','','','','')
    ### MENSAGEM #########
    mensagem()
    ######################
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                            if not regexs == None: #<externallink> and <regex>
                                ...
                                #addLink(url[0],name.encode('utf-8', 'ignore')+  '[COLOR firebrick]build XML[/COLOR]',thumbnail,fanArt,desc,genre,date,True,None,regexs,total)
                            else:
...
def urlsolver(url):
    ...
    ##genesis_url = 'https://raw.githubusercontent.com/lambda81/lambda-addons/master/plugin.video.genesis/commonresolvers.py'
        genesis_url = 'https://raw.githubusercontent.com/FenixFlixHD/OneRepo/master/plugin.video.genesis/commonresolvers.py'
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,year,director,duration,premiered,studio,rate,originaltitle,country,rating,userrating,votes,aired,showcontext=False,regexs=None,reg_url=None,allinfo={}):
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from FenixFlix Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if showcontext == '!!update':
                fav_params2 = (
                    '%s?url=%s&mode=17&regexs=%s'
                ...
                contextMenu.append(('[COLOR firebrick]!!update[/COLOR]','XBMC.RunPlugin(%s)' %fav_params2))
            if not name in FAV:
                fav_params = (
                    '%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                    %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), urllib.quote_plus(fanart))
                ...
                if regexs:
                    fav_params += "&regexs="+regexs
                ##contextMenu.append(('Adicionar [COLOR firebrick]FenixFlix[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
                        ## %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage), urllib.quote_plus(fanart), mode)))
            liz.addContextMenuItems(contextMenu)
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz,isFolder=True)
        return ok
...
#    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    xbmcgui.Dialog().ok('[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B] [B][COLOR white]AVISO[/COLOR][/B]','Baixe ou solicite o App [COLOR firebrick]P2P[/COLOR] para o acesso Premium!')
    xbmc.executebuiltin("XBMC.Container.Refresh")
    
elif mode==42:
    addon_log("infovip")    
    xbmcgui.Dialog().ok(titulo_vip, descricao(url_vip_dialogo))
    xbmcaddon.Addon().openSettings()
    xbmcgui.Dialog().ok('[B][COLOR firebrick]FENIX[/COLOR] [COLOR white]FLIX[/COLOR][/B] [B][COLOR white]AVISO[/COLOR][/B]','Baixe ou solicite o App [COLOR firebrick]P2P[/COLOR] para o acesso Premium!')
